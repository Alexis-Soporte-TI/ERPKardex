...
namespace ERPKardex.Controllers
{
    ...
    public class ReqCompraController : BaseController
    {
        ...
        public async Task<JsonResult> GetDetalleParaEdicion(int id)
        {
            try
            {
                var cabecera = await (from r in _context.ReqCompras
                                      join est in _context.Estados on r.EstadoId equals est.Id
                                      where r.Id == id
                                      select new
                                      {
        ...
        [HttpPost]
        public JsonResult Actualizar(int id, ReqCompra cabecera, string detallesJson)
        {
            using (var transaction = _context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    // Reemplazar detalles
                    var detallesAnteriores = _context.DReqCompras.Where(d => d.ReqCompraId == id).ToList();
                    _context.DReqCompras.RemoveRange(detallesAnteriores);
